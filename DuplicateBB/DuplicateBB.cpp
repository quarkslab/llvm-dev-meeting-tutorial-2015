...
static llvm::cl::opt<Ratio> DuplicateBBRatio {
  "duplicate-bb-ratio",
  llvm::cl::desc("Only apply the duplicate basic block pass on <ratio> of the basic blocks"),
...
class DuplicateBB : public llvm::FunctionPass {
      ...
      for(BasicBlock& BB : F) {
        ...
        if(Dist(RNG) <= Ratio) {
          ...
          if (ReachableValuesCount) {
            ...
            // Store the binding and a BB to duplicate and the context variable used to hide it
  ...
  private:
    ...
    void duplicate(BasicBlock &BB, Value* ContextValue, std::map<Value*, Value*>& ReMapper) {
      ...
      for (Instruction *I : TailInstructions) {
        Instruction &Instr = *I;
        assert(not isa<PHINode>(&Instr) and "phi nodes have already been filtered out");
...
static RegisterPass<DuplicateBB> X(
    "duplicate-bb",
