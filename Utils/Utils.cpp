...
bool parser<Ratio>::parse(Option &O, StringRef ArgName, const std::string &Arg,
                          Ratio &Val) {
  ...
  if (EndPtr == ArgCStr) {
    return O.error(ArgName + " value `" + Arg +
...
void parser<Ratio>::printOptionDiff(const Option &O, Ratio const &,
                                    OptionValue<Ratio>,
