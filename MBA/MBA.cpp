 ...
 * (an add between two integers) by an equivalent instruction that is more complex.
 *
 * This showcases the development of a simple Basic Block pass.
 */

/* for debug support
 * see http://llvm.org/docs/ProgrammersManual.html#the-debug-macro-and-debug-option
 */
        ...
        if (auto *BinOp = dyn_cast<BinaryOperator>(&Inst)) {
          ...
          if (Dist(RNG) < MBARatio.getValue().getRatio()) {
            ...
            if (Opcode == Instruction::Add and BinOp->getType()->isIntegerTy()) {
              ...
              // We could use ReplaceInstWithValue  but that's incomaptible with inplace modification of the BB
 ...
 */
char MBA::ID = 0;
static RegisterPass<MBA> X(
    "mba", // the option name -> -mba
...
// Note the registration point, clang offers several insertion point where you can insert your pass
